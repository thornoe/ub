...
\begin{figure}[H]
  ...
  \includegraphics[width= \textwidth]{03_figures/fig2a}
  \label{fig:gaussian}
  \vspace{-2cm}
\end{figure}
\textbf{\textit{\begin{itemize}
...
\end{itemize}}}\noindent
The Moving Average processes are likewise generated with a sample size at 500m including a burn-in period of 2.000 iterations. The time series are plotted in figure \ref{fig:ma1} and the autocorrelation functions and partial autocorrelation functions are plottes in figure \ref{fig:ma1_acf}.
\\ \\
...
The ACFs in figure \ref{fig:ma1_acf} confirms that there are not any persistence after the 1st lag, while the PACFs show significant lags besides the first but with alternating sign. For $\theta=0.5$ $x_t$ is linearly correlated with $x_{t-1},\ x_{t-2}$ and $x_{t-1}$ and the MA(1) process with $\theta=0.5$ shows significant correlation with lags 1-5 and 7, as well as border-line significant correlation with lags 6 and 8.
